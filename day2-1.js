...
let sum=0;
let compare = function (x,y) { return (x===y) };
...
let i=0, j=0, min, max;
// each row is indexed by "i"
// each col is indexed by "j"

for (i=0;i<input.length;i++) {
    console.log("row["+i+"]: "+input[i]);
    min = max = input[i][0];
    for (j=0;j< input[i].length;j++) {
        console.log("col["+j+"]: "+input[i][j]);
...
//     let a = i;
//     let b = (i+digits.length/2) % digits.length;
//     console.log("a:"+i+", b:"+b);
//     if (compare(digits[a],digits[b])) {sum += +digits[i];}
// }
console.log("sum: "+sum);
