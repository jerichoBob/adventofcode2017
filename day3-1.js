...
let findDistance2 = function(input_val) {
    let sqrt = Math.ceil(Math.sqrt(input_val));
    let base_hi = (sqrt % 2 === 0) ? sqrt+1 : sqrt;
    let base_lo = base_hi - 2;
    let n = Math.ceil(base_lo / 2);

    let range_lo= base_lo*base_lo; // 9 & 25

    let dist_min=n, dist_max=2*n; // 2 & 4

    let index = (input_val - range_lo) % dist_max;
...
//     let distance = findDistance2(val);
//     console.log("input_val: " + val+", distance: " + distance);
// }

let input = 368078; //368078
let distance = findDistance2(input);
