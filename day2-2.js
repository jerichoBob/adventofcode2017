...
let sum=0;
// return a boolean
let isDivisible = function(x, y) {
    return (x % y === 0)
};

// return a number
let divisbleFactor = function(x,y) {
    if (isDivisible(x,y)) { return (x/y) }
    else if (isDivisible(y,x)) { return (y/x) }
    else { return 0 }
};
...
let i=0, j=0, k=0, dig1 = 0, dig2 = 0;


for (i=0;i<input.length;i++) { // each row is indexed by "i"
    console.log("row["+i+"]: "+input[i]);

    for (j=0;j< input[i].length-1; j++) { // digit1 index
        for (k=j+1; k<input[i].length; k++) { // digit2 index
            dig1 = input[i][j];
            console.log("dig1: "+ dig1);
            dig2 = input[i][k];
            console.log("dig2: "+ dig2);
            let factor = divisbleFactor(dig1,dig2)
